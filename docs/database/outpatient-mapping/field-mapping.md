...
| `patients` | `medical_record_number` | varchar(50) | No | Unique | `ref_pasien` | `no_rm` | varchar(50) | renamed | Preserve legacy value; generate six digits only for patient baru | Tinggi | Nilai legacy tidak dibatasi enam digit |
...
| `patients` | `status` | enum('active','deleted') | No | Default `active` | N/A | N/A | N/A | new | Status soft delete pasien | Tinggi | Approved supervisor 2026-09-17 |
| `outpatient_admissions` | `registration_number` | varchar(50) | No | Unique | N/A | N/A | N/A | derived | Generate `RJ-YYYYMMDD-0001` per `service_date` | Tinggi | Satu nomor registrasi/kunjungan baru |
| `outpatient_admissions` | `service_date` | date | No | Composite unique | `trx_admisi` | `waktu_admisi` | datetime | transformed | Ambil komponen tanggal pelayanan | Tinggi | Bersama registration dan poli mencegah duplikasi admission |
| `outpatient_admissions` | `guarantor_id` | int | Yes | FK | `trx_admisi` | `id_jenis_asuransi` | int | reference | Map ke master guarantors | Sedang | Penjamin transaksi melekat pada admission |
...
| `outpatient_registrations` | `id` | bigint | No | PK, AI | N/A | N/A | N/A | new | Generate | Tinggi | Tabel registrasi terstruktur baru tanpa nomor bisnis terpisah |
